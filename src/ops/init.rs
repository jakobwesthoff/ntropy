...
pub fn init_vault(path: &Path) -> Result<InitReport> {
    ...
    // The root README identifying the directory as an ntropy vault, written
    // only if absent so a user-edited README is kept.
    ensure_file(&layout.readme_file(), VAULT_README, &mut created)?;
