...
/// the whole tree and regenerating it prunes any stale or orphaned links for
/// free. Removing a missing directory is not an error.
pub fn wipe_and_recreate_dir(path: &Path) -> Result<()> {
    match std::fs::remove_dir_all(path) {
        Ok(()) => {}
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => {}
        Err(e) => return Err(FsError::new("wiping directory", path, e)),
    }
    create_dir_all(path)
}
...
mod tests {
    ...
    #[test]
    fn wipe_and_recreate_dir_clears_contents() {
        let dir = tempfile::tempdir().expect("temp dir");
        let view = dir.path().join("by-tag");
        std::fs::create_dir_all(view.join("rust")).expect("seed tree");
        std::fs::write(view.join("rust").join("a.md"), b"x").expect("seed file");

        wipe_and_recreate_dir(&view).expect("wipe");

        assert!(view.is_dir());
        assert_eq!(std::fs::read_dir(&view).expect("read dir").count(), 0);
    }

    #[test]
    fn wipe_and_recreate_dir_creates_missing_dir() {
        let dir = tempfile::tempdir().expect("temp dir");
        let view = dir.path().join("by-status");
        wipe_and_recreate_dir(&view).expect("wipe missing");
        assert!(view.is_dir());
    }
